import { SalesOrderDto, Paged, formatMoney, formatDate } from '@erp/shared';
...
function StatusBadge({ status }: { status: string }) {
  ...
  if (status === 'CONFIRMED') return <Badge tone="info">Đã chốt</Badge>;
  if (status === 'PARTIALLY_ISSUED') return <Badge tone="warning">Đang giao</Badge>;
  if (status === 'ISSUED') return <Badge tone="success">Đã giao đủ</Badge>;
...
export default async function SalesOrdersPage({ searchParams }: Props) {
  ...
  const res = await api.get<Paged<SalesOrderDto>>('/sales-orders', { page, pageSize: '20', q, status });
  ...
  const columns: Column<SalesOrderDto>[] = [
    { 
      ...
      header: 'Số SO', 
      render: (r) => (
        <Link href={`/sales/orders/${r.id}`} className="font-semibold text-primary hover:underline">
          {r.soNo}
        </Link>
      ) 
    },
  ...
  return (
    <>
      <PageHead
        breadcrumb={[{ label: 'Bán hàng' }, { label: 'Đơn bán hàng' }]}
        title="Đơn bán hàng"
      />
        ...
        <div className="flex items-center justify-between mb-4">
          <form className="flex items-center gap-3">
            ...
            >
              ...
              <option value="PARTIALLY_ISSUED">Đang giao</option>
              <option value="ISSUED">Đã giao đủ</option>
            ...
            {(q || status) && (
              <Link href="/sales/orders" className="text-sm text-text-muted hover:text-text no-underline">
                Xóa lọc
              </Link>
            )}
          </form>
          <Link href="/sales/orders/new" className="h-9 px-4 inline-flex items-center justify-center rounded-md bg-primary text-white text-sm font-semibold hover:bg-primary-hover no-underline shadow-sm">
            Tạo đơn bán hàng
          </Link>
        </div>
          ...
          footer={
            res.total > res.pageSize && (
              <Pagination
                ...
                total={res.total}
                baseUrl={`/sales/orders?q=${q}&status=${status}`}
              />
            )
          }
        />
